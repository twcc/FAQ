...
:::spoiler Q1. 如何暫停容器？
:::info
目前系統不支援容器暫停的功能，下列有兩個方式可以替代暫停容器，您可擇一使用
1. 您可製作容器複本保留工作環境，並刪除容器，待需要使用容器時再以複本建立新容器。
2. 您可參考[<ins>此文件</ins>](https://man.twcc.ai/@twccdocs/howto-cli-ccs-automate-compute-delete-with-twccli-zh)，編寫腳本達到自動化建立容器、執行訓練再刪除容器。
:::
