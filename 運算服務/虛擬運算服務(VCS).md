...
:::spoiler Q7. 想了解2022/11/25前建立的虛擬運算個體系統碟收費方式？

:::info

於2022/11/25前建立的系統碟提供100GB(含)不收費的優惠，超過100GB的系統碟比照虛擬磁碟HDD類型費率計費

:::

:::spoiler Q8. 停止虛擬運算個體時系統碟會不會計費?

:::info

從使用者介面與指令停止虛擬運算個體時(狀態為stopped)，將停止虛擬運算個體與系統磁碟計費。如果是在虛擬運算個體中執行關機指令(個體狀態為shutdown)，系統仍為此虛擬運算個體保留資源，因此系統磁碟與虛擬運算個體將維持計費。
...
:::spoiler Q9. 大容量系統磁碟映像檔是否可以複製到其他專案使用?

:::info

目前僅支援100GB系統磁碟，建議您先使用100GB系統磁碟建立映像檔，將映像檔複製到目的專案後，再使用映像檔建立較大系統磁碟容量的虛擬運算個體。
